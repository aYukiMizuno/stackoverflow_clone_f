...
下記のようにして取得できます。(`StackoverflowCloneF`部分は各自のgearの名前に応じて変更ください。)
* root key: `StackoverflowCloneF.Dodai.root_key()`
* app key: `StackoverflowCloneF.Dodai.app_key()`
* app_id: `StackoverflowCloneF.Dodai.app_id()`
* group_id: `StackoverflowCloneF.Dodai.default_group_id()`
...
```
StackoverflowCloneF.Helper.ErrorJson.json_by_error(conn, StackoverflowCloneF.Error.BadRequestError.new())
```

これは`StackoverflowCloneF.Error.BadRequestError.new()`は下記のstructを生成しますが、
```
%StackoverflowCloneF.Error.BadRequestError{
  code:        "400-06",
  description: "Unable to understand the request.",
  name:        "BadRequest",
  source:      "gear"
...
```
`StackoverflowCloneF.Helper.ErrorJson.json_by_error/2`はこの`code`や`description`の値を元に`conn`を組み立ててくれるためです。
