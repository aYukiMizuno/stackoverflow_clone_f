...
StackoverflowCloneFは[stackoverflow](https://stackoverflow.com/)のクローンサイトである。
StackoverflowCloneFでは、ユーザ同士が技術トピックに対して、質問、回答を投稿することができるQ&Aサイトである。
下記では、StackoverflowCloneFにおける要件を記載する。

## 用語の定義

StackoverflowCloneFで用いる用語について定義する。

* User
  * StackoverflowCloneFを利用する利用者。
...
下記の要件を満たすQuestionの一覧を表示するためのページ。このページをStackoverflowCloneFのトップページとする。
